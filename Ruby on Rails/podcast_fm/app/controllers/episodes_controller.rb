class EpisodesController < ApplicationController
  ...
  def show
    @episodes = Episode.where(podcast_id: @podcast.id).order("created_at DESC").limit(3).reject{|e| e.id == @episode.id}
  end
  ...
  def require_permission
    ...
    if current_podcast != @podcast
      redirect_to root_path, notice: "you're not allowed to view this page"
    end
  end
end
